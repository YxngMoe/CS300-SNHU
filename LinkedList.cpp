...
//if head if null still then make it equal to newNode
if(head == nullptr){
head = newNode;
tail = newNode;
}else{
//making sure the tail is pointing at noce
tail->next = newNode;
//then the tail becomes that newNode;
tail = newNode;
}
//increase size by one each time
size++;
}
